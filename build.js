...
await Promise.allSettled(emails.map(async (emailName) => {
  ...
  await fs.writeFile(path.join('dist', emailName, 'index.html'), html, {encoding: 'utf-8'});

  const assets = (await fs.readdir(path.resolve("content", emailName))).filter(filename => !filename.endsWith('.mjml'));

  await Promise.allSettled(assets.map(async (filename) => 
    await fs.copyFile(
